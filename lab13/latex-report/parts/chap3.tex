...
В соседнем репозитории \texttt{home-server} добавлен скрипт
\texttt{scripts/51-deploy-marvel-site.sh}. Он идемпотентный --- то
есть может быть безопасно перезапущен любое число раз, без побочных
эффектов. Шаги работы:

\begin{enumerate}
  \item Запрашивает у пользователя домен (по умолчанию ---
        \texttt{marvel.server34.netcraze.club}) и путь к исходникам
        сайта на локальной файловой системе.
