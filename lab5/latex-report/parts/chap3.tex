...
\texttt{forwarders}~--- Google DNS для внешних зон;
\texttt{listen-on}~--- принимать запросы только с localhost и LAN-интерфейса.

\subsection{named.conf.local}
\begin{lstlisting}
...
Файлы зон в \texttt{/var/lib/bind/}: здесь BIND имеет права на запись,
что необходимо для DDNS (журнальные файлы \texttt{.jnl}).
...
\textbf{Обновление /etc/dhcp/dhcpd.conf:}
\begin{lstlisting}
...
\textbf{Перезапуск и проверка DDNS:}
\begin{lstlisting}
systemctl restart bind9
