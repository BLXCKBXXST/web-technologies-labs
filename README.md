...
**Шаг 1.** В VirtualBox → Настройки ВМ → Общие папки → Добавить:
...
После перезагрузки папка появится автоматически в `/media/sf_<имя_папки>`. Все клоны этого образа будут иметь доступ к папке без повторной настройки — достаточно привязать папку в настройках клона.
