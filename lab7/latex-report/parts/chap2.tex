...
nslookup gateway.yazikov.iks531.local
\end{lstlisting}
...
\begin{figure}[h]
  ...
  \caption{Файл \texttt{/etc/hosts} с FQDN-именем \texttt{mail.yazikov.iks531.local}}
...
nslookup mail.yazikov.iks531.local
\end{lstlisting}

% — Скриншот: nslookup mail.yazikov.iks531.local (успешно) —
\begin{figure}[h]
  \centering
  \includegraphics[width=0.85\textwidth]{img/05_dns_mail_record}
  \caption{Проверка DNS-записи \texttt{mail.yazikov.iks531.local}}
  \label{fig:dns_mail}
\end{figure}
